...
def clean_sqlite_db(conn : sqlite3.Connection) -> None:
    """
...
def is_lookup_key_valid_for_sqlite_db(word : str) -> bool:
    #if re.match('^[ a-zA-Z0-9\'"?`’1√,;:.ḥḍḌṭṬāĀūŪīĪḷḶṇṆñÑṅṄŋṁṃ-]+$', word):
    if not word.count("<") and not word.count(">"):
        return True
    return False
...
def write_new_inflected_form_to_sqlite_db(conn : sqlite3.Connection, word: str, link_text: str):
    """
    Write a new inflected form to the data base
    """
    sql_words_entry = ''' INSERT INTO dict_inflected_form(inflected_form,link_text) VALUES(?,?) '''
    cur = conn.cursor()
...
def write_new_entry_to_sqlite_db(conn : sqlite3.Connection, word: str, entry: str, headword_table_name_wo_app_prefix : str) -> int:
    ...
    """
    #if not is_lookup_key_valid_for_sqlite_db(word):
        #print("filtered out entry with invalid key: " + word)
        #return 0
...
def check_links_in_sqlite_db(conn : sqlite3.Connection) -> int:
    cur = conn.cursor()
    cur.execute("SELECT inflected_form, link_text FROM dict_inflected_form")
    error_cnt = 0
    for row in cur:
        key = row[0]
        link_target = row[1]
        if not headword_exists_in_sqlite_db(conn, link_target):
            error_cnt += 1
            print("error: missing word link target: '" + key + "' -> '" + link_target + "'")
    return error_cnt
...
def write_mdict_to_sqlite_db(conn : sqlite3.Connection, mdx, replace_text_for_mdict, limit : int, headword_table_name_wo_app_prefix : str):
    ...
    print("wrote " + str(cnt_hw) + " entries in total")
    conn.commit()
    if limit == 0:
        pass
        #broken_links_cnt = check_links_in_sqlite_db(conn) # takes very very long...
...
if __name__ == '__main__':
    ...
    parser.add_argument('-y', '--file-per-entry',
                        help='when --extract is given then create one file per entry in the data folder')
    ...
    if args.extract:
        ...
        if mdx:
            if args.word_list != "":
                ...
                print("extracting one file per entry")
                cnt = 0
                for key, value in mdx.items():
                    output_fname = ''.join([datafolder, "/", key.decode(), os.path.extsep, 'html'])
                    ...
                    tf.close()
                    cnt += 1
                    if headword_count_limit and cnt >= headword_count_limit:
                        break
